      ...
      <div id="value"></div>
        ...
        <div class="container">
          <form action="filtropreco.php" method="POST">
            <input type="range" min="0" max="700" value ="<?php echo $valor = $_POST['valor']?>" class="slider" id="slider" name="valor">
...
<?php
                ...
                $valor = $_POST['valor'];
                $resultado =mysqli_query( $conexao,"SELECT * FROM cadastro_produtos WHERE preco <= $valor ORDER BY preco ASC");
                if (mysqli_num_rows($resultado) > 0) {
                    while ($GetProduto = mysqli_fetch_assoc($resultado)) {
          
                         echo "<div class='col-sm-6 col-lg-4 text-center item mb-4'>";
                         echo "<a href='produto.php?id=".$GetProduto['id']."'>";
                         echo '<img src="data:image/png;base64,'. base64_encode($GetProduto['foto']).'" width = 250 heigth = 250 />';
                         echo "<h3 class='text-dark'><a href='#'>" . $GetProduto['nome_produto'] . "</a></h3>";
                         echo "<p class='price'>R$ " . $GetProduto['preco'] . "</p>";
                         echo "</div>";
                  }
                }
        ?>
